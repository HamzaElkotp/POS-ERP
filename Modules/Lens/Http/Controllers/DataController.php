...
class DataController extends Controller
{
    ...
    public function superadmin_package()
    {
        return [
            [
                'name' => 'lens_module',
                'label' => __('lens::lang.lens_module'),
                'default' => false,
            ],
        ];
    }
    ...
    public function modifyAdminMenu()
    {
        ...
        // if (auth()->user()->can('accounting.access_accounting_module') && $is_accounting_enabled) {
            Menu::modify(
                'admin-sidebar-menu',
                function ($menu) {
                    $menu->url(action([\Modules\Lens\Http\Controllers\LensController::class, 'dashboard']), __('lens::lang.lens'), ['icon' => 'fas fa-money-check fa', 'style' => config('app.env') == 'demo' ? 'background-color: #D483D9;' : '', 'active' => request()->segment(1) == 'lens'])->order(50);
                }
            );
        // }
    }
