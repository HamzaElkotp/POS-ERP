...
class InstallController extends Controller
{
    ...
    {
        $this->module_name = 'spreadsheet';
        $this->appVersion = config('spreadsheet.module_version');
    }
    ...
    public function index()
    {
        ...
        //Check if Spreadsheet installed or not.
        $is_installed = System::getProperty($this->module_name.'_version');
        if (! empty($is_installed)) {
            abort(404);
        }

        $action_url = action([\Modules\Spreadsheet\Http\Controllers\InstallController::class, 'install']);
    ...
    public function install()
    {
        try {
            ...
            Artisan::call('module:migrate', ['module' => 'Spreadsheet', '--force' => true]);
            Artisan::call('module:publish', ['module' => 'Spreadsheet']);
            System::addProperty($this->module_name.'_version', $this->appVersion);

            DB::commit();

            $output = ['success' => 1,
                'msg' => 'Spreadsheet module installed succesfully',
            ];
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update()
    {
        ...
        try {
            ...
            $output = ['success' => 1,
                'msg' => 'Spreadsheet module updated Succesfully to version '.$this->appVersion.' !!',
            ];
