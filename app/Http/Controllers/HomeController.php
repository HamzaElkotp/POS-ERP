...
class HomeController extends Controller
{
     ...
     */
    public function getTransSearch(Request $request){
        $val = $request->val ;
        $files_names = [
            'account',
            'auth',
        ...
        foreach($files_names as $name){
            $files[$name]['en'] = \File::getRequire(base_path().'/lang/en/'.$name.'.php');
            $files[$name]['ar'] = \File::getRequire(base_path().'/lang/ar/'.$name.'.php');
        }

        $arr = [];
    
        // foreach($files as $key=>$file) {
        //     $result = collect($files[$key]['en'])->contains(function ($value) use ($val) {
        //         if(is_string($value)){
        ...
        //     });
            foreach ($files as $key => $file) {
               foreach ($files[$key]['en'] as $key2 => $value) {
                if(is_string($value) && \Str::contains($value, $val)){
                    $arr[] = $key . ' / ' . $key2 . ' / ' . $value;
                }
               }
            }
    ...
    public function update_trans($file,Request $request){
        ...
        foreach ($data as $key => $value) {
            ...
            $word = substr($key, 0, -3);
            if($lang == 'en')
               $str_of_arr_en .= " '".$word."' => '".$value."', \n";


            if($lang == 'ar')
               $str_of_arr_ar .= " '".$word."' => '".$value."', \n";
        }

        $current_en = "<?php \n\n\n\n\n return \n\n [ \n\n ".$str_of_arr_en." \n\n ];"  ;
        $current_ar = "<?php \n\n\n\n\n return \n\n [ \n\n ".$str_of_arr_ar." \n\n ];"  ;

        file_put_contents(base_path().'/lang/en/'.$file.'.php', $current_en);
        file_put_contents(base_path().'/lang/ar/'.$file.'.php', $current_ar);
    ...
    public function all_trans(){
        ...
        return view('translation.index', compact('files'));
    ...
    public function getPurchasePaymentDues()
    {
        if (request()->ajax()) {
            ...
            return Datatables::of($dues)
                ...
                ->addColumn('action', '@can("purchase.create") <a href="{{action([\App\Http\Controllers\TransactionPaymentController::class, \'addPayment\'], [$id])}}" class="btn btn-xs btn-warning add_payment_modal"><i class="fas fa-money-bill-alt"></i> @lang("purchase.add_payment")</a> @endcan')
