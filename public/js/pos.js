$(document).ready(function () {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function () {
        ...
        var purch = __read_number(tr.find('input.purch'));
        var line_total = entered_qty * unit_price_inc_tax;
        var purch_total = entered_qty * purch;

        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
        __write_number(tr.find('input.pos_line_purch_total'), purch_total, false, 2);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        tr.find('span.pos_line_purch_total1').text(__currency_trans_from_en(purch_total, true));
...
function pos_total_row() {
    ...
    var price_purch_total = get_subtotal_purch();
    ...
    $('#tot_purch').val(__currency_trans_from_en(price_purch_total, false));
    $('#price_total').val(__currency_trans_from_en(price_total, false));
...
function get_subtotal_purch() {
    var price_purch_total = 0;

    $('table#pos_table tbody tr').each(function () {
        price_purch_total = price_purch_total + __read_number($(this).find('input.pos_line_purch_total'));
    });

    //Go through the modifier prices.

    return price_purch_total;
}
